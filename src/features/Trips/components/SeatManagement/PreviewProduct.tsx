...
import { Drawer, Space, Table, Button as ButtonAntd, Form, message, Row, Col, Input, Button, Select, Tag } from 'antd'
...
const PreviewProduct = () => {
  ...
  const fetchSeat = async () => {
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_API}/trips/${product?._id}/busseats`, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      })
      setSeat(data?.seats)
      console.log('dữ liệu log ra:', data)
    } catch (error) {
      console.error(error)
    }
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (sizes: any) => (
        <>
          <Tag color={sizes == 'EMPTY' ? 'green' : 'error'} className=''>
            {sizes == 'EMPTY' ? 'Hoạt động' : 'Đã bán'}
          </Tag>
        </>
      )
    },
